  ...
  <body>
   	...
   	<div class="container">
   		...
   		<div class="span9">
   			...
   			<div class="wall" style="height:600px;">
   				<p>添加新记录：</p>
				<form action="contact.jspx?_c=savejilu" method="post">
					<input type="hidden" value="${contact.id }" name="contactid">
					<textarea rows="5" cols="500" style="width:667px" name="neiRong"></textarea><br>
					<input type="submit" class="btn btn-info" value="发布">
				</form>
				<p>所有记录：</p>
				<c:if test="${fn:length(noteList) == 0 }">
					<div>现在还没有人记录，赶快发表记录吧</div>
				</c:if>
				<c:choose>
						...
						<c:forEach items="${noteList }" var="note">
							<c:forEach items="${userList }" var="user">
								<c:if test="${user.id == note.userid }">
									<blockquote>
									${note.neiRong }
									<small>${user.username }发布于${note.createtime }</small>
									</blockquote>
								</c:if>
							</c:forEach>
						</c:forEach>
